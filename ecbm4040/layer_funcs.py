...
def conv2d_forward(x, w, b, pad, stride):
    ...
    # Size of the output
    Hh = int(1 + (H + 2 * pad - FH) / stride)
    Hw = int(1 + (W + 2 * pad - FW) / stride)
    
    out = np.zeros((N, Hh, Hw, F))
            ...
            for k in range(Hh):
                ...
                for l in range(Hw):
                    ...
                    out[n, k, l, f] = np.sum(
                        ...
                        x_pad[n, k * stride:k * stride + FH, l * stride:l * stride + FW,:] * w[f, :]) + b[f]
...
def conv2d_backward(d_top, x, w, b, pad, stride):
    ...
    N, H, W, C = x.shape
    F, HH, WW, C = w.shape
    N, Hh, Hw, F = d_top.shape

    # For dw: Size (HH, WW, C)

    dw = np.zeros((F, HH, WW, C))
    
    for fprime in range(F):
        for cprime in range(C):
            for i in range(HH):
                for j in range(WW):
                    sub_xpad = x_pad[:, i:i + Hh * stride:stride, j:j + Hw * stride:stride, cprime]
                    dw[fprime, i, j, cprime] = np.sum(
                        d_top[:, :, :, fprime] * sub_xpad)
