...
class RMSpropOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            params[k] = params[k] + -learning_rate * grads[k] / (np.sqrt(cache[k]) + eps)
...
class AdamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            momentum_bias = momentums[k] / (1 - beta1**t)
            velocity_bias = velocitys[k] / (1 - beta2**t)
            params[k] = params[k] + -learning_rate * momentum_bias / (np.sqrt(velocity_bias) + eps)
