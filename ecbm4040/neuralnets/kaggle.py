...
class conv_layer(object):
    def __init__(self, input_x, in_channel, out_channel, kernel_shape, rand_seed, index=0):
        ...
        with tf.variable_scope('conv_layer_%d' % index):
            ...
            #cell_out = tf.nn.relu(norm_layer(conv_out + bias).output())
    ...
    def __init__(self, input_x, in_size, out_size, rand_seed, activation_function=None, index=0):
        ...
        with tf.variable_scope('fc_layer_%d' % index):
            ...
            if activation_function is not None:
                ...
                #cell_out = activation_function(norm_layer(cell_out).output())
...
def my_LeNet(input_x, input_y,
    ...
    batch_norm_out_0 = norm_layer(conv_layer_0.output())

    pooling_layer_0 = max_pooling_layer(input_x=batch_norm_out_0.output(),
    ...
    batch_norm_out_1 = norm_layer(conv_layer_1.output())

    pooling_layer_1 = max_pooling_layer(input_x=batch_norm_out_1.output(),
                                        k_size=pooling_size[0],
                                        padding="VALID")
